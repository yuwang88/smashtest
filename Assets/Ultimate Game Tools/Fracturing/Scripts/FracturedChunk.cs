...
public class FracturedChunk : MonoBehaviour
{
    ...
    void OnCollisionEnter(Collision collision)
    {
        ...
        if(IsDetachedChunk == false)
        {
            ...
            if(bOtherIsFreeChunkFromSameObject == false && collision.relativeVelocity.magnitude > FracturedObjectSource.EventDetachMinVelocity && fMass > FracturedObjectSource.EventDetachMinMass && rigidbody != null && IsSupportChunk == false)
            {
                ...
                if(collisionInfo.bCancelCollisionEvent == false)
                {
                    ...
                    foreach(FracturedChunk chunk in listBreaks)
                    {
                        ...
                        if(collisionInfo.bCancelCollisionEvent == false)
                        {
                            ...
                            chunk.rigidbody.AddExplosionForce(collision.impactForceSum.magnitude * FracturedObjectSource.EventDetachExitForce, collision.contacts[0].point, 0.0f, FracturedObjectSource.EventDetachUpwardsModifier);
                        }
                    }
                }
            }
    ...
    {
        if(rigidbody != null && IsSupportChunk == false)
        {
            List<FracturedChunk> listBreaks = new List<FracturedChunk>();
    ...
    {
        if(IsSupportChunk == false && IsDetachedChunk == false && rigidbody)
        {
